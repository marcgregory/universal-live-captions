...
- **Commit:** (referenced in the v0.5.39 release-close-out commit) — 6 files (+214/-0).
...
### Installer-first distribution, install-path auto-resolution, and readme UX split (2026-08-10)

- **Installer is now the recommended artifact; portable ZIP ships alongside.** End users download `UniversalCaptions-Setup-0.5.31.exe`, double-click, press START. Advanced users extract `UniversalCaptions-0.5.31-win-x64-full.zip` and run `launcher.cmd`. Both ship the **same staged closure** (single `Stage` tree → both outputs), so runtime and model behavior are identical regardless of install path. New stage 6/7 in `packaging/build-package.ps1` builds the portable ZIP via `System.IO.Compression.ZipFile::CreateFromDirectory` (built into .NET 8, no new tooling); `-SkipZip` mirrors the existing `-SkipSetup` / `-SkipPublish` switches. Setup.exe version bumped from 0.5.25 → 0.5.31 in `packaging/UniversalCaptions.iss`. See `docs/DEPLOYMENT.md` for the artifacts table and the build command.
- **App auto-resolves the bundled Python.** New `src/UniversalCaptions.App/InstallPathResolver.cs` is called from both `SpeechEngineFactory.ResolveFasterWhisperPython` and the Argos wiring in `App.xaml.cs`. Resolution chain (highest priority first): `UC_FW_PYTHON` / `UC_ARGOS_PYTHON` env var (set by `launcher.cmd`) → `<install>/py/python.exe` (sibling of the .exe via `AppContext.BaseDirectory`, **new in v0.5.31**) → legacy `%TEMP%\fwv` / `%TEMP%\argosv` dev venv → system `python`. Net effect: a portable-ZIP user who runs `UniversalCaptions.App.exe` directly without `launcher.cmd` now resolves the bundled runtime automatically, and the install is one step more robust if the launcher ever fails to set its env vars. Existing `dotnet run` developers with a pre-staged `%TEMP%\fwv` venv see no behavior change. `BaseDirectoryAccessor` is an `internal static` test seam (production always reads `AppContext.BaseDirectory`); 7 deterministic `InstallPathResolverTests` pin the chain end-to-end.
- **User-facing readme split.** `src/UniversalCaptions.App/readme.txt` and the top-level `README.md` no longer instruct end users to create `%TEMP%\fwv` / `%TEMP%\argosv` venvs or set up Python / .NET manually. Both lead with the 4-step installer flow and a privacy callout. All dev material — venv setup, the full env-var table, `dotnet build` / `dotnet test` / `dotnet run` / `dotnet publish` quickstart, dev troubleshooting — moved to the new `docs/DEVELOPER_SETUP.md`. End-user docs are now aggressively simple; developer docs are unchanged in content but moved out of the user-facing path.
- **No production behavior change.** Frozen defaults (production `fasterwhisper-native` + live partials, `UC_NATIVE_THREADS=4`, `MaxSegmentDuration=8 s`, 8 s segment cap, partial cadence 1 s / 4 s, worker wire protocol, Argos path, settings store, ADR-0007/0008) all unchanged. Benchmarks keep the `%TEMP%\fwv` / `%TEMP%\argosv` discovery unchanged (dev-only path).
- **Caveat:** `bin/Release/net8.0-windows/{win-x64,win-x64/publish}/readme.txt` are stale build-output copies; they regenerate on next `dotnet publish`. Not hand-edited. Documented in INSTALLER_DISCOVERY §10.
- **Tests + verification:** 7 new `InstallPathResolverTests` (env-wins, install-sibling, dev-venv, system-fallback, Argos-independent-name, whitespace-env, shared-install-sibling). **564/564 tests** (106 Audio + 72 Captions + 111 Speech + 42 Translation + 131 App + 102 Speech.Gemini), Release 0 warnings/0 errors, `dotnet format` clean.
