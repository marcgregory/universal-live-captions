...
2026-08-13 — `dotnet build UniversalCaptions.slnx` succeeded, 0 warnings, 0 errors. `dotnet test UniversalCaptions.slnx` passed **621/621** (107 Audio + 81 Captions + 111 Speech + 42 Translation + 161 App + 119 Speech.Gemini), `dotnet format` clean. **Real-app smoke PASS (2026-08-13) — v0.5.37 mixed-language history scrub** on the Release app + WASAPI loopback + Gemini provider: in-session sequence with no Stop/Start — Translate OFF + English source captions visible → Translate ON → Tagalog → target switch `tl → ja` (previous Tagalog history cleared, new JA session starts) → Translate OFF (committed Tagalog/Japanese history cleared, English SourceStt history preserved). Visual evidence in-session. See CHANGELOG v0.5.37, RELEASE_PLAN §3.6, TEST_REPORT (2026-08-13). Prior (2026-08-12): runtime Gemini-toggle latency verification (CHANGELOG v0.5.35, measurement only — no code change). Prior (2026-08-12): v0.5.33 translation parity 22/22 (Argos 11/11 + Gemini 11/11) — see CHANGELOG v0.5.33, TEST_REPORT (2026-08-12), `v0533_parity_acceptance.log` (untracked). Prior (2026-08-06): Entry 16 `UC_NATIVE_THREADS` decode cap (Threads=4), Entry 15 overlay live-line, Entry 14 default promotion (ADR-0008) — see CHANGELOG v0.5.22–v0.5.25, TEST_REPORT (Entry 15/16 + final acceptance), BENCHMARK_REPORT (Entry 16 gate).
