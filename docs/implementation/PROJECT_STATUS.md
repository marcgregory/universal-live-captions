...
Slice 5 — WPF overlay + control window (render `CaptionState`, consume `ICaptionService` events on the dispatcher) — **in progress**: implementation + unit tests complete (2026-08-01); manual overlay/device verification **completed 2026-08-01** (real capture → Whisper → overlay, interaction, lifecycle, error paths); real-Argos wiring pending (dev Argos venv unavailable on this machine).
...
Finishing Slice 5 close-out: the manual overlay/device verification is complete (2026-08-01 — real capture → Whisper → overlay, always-on-top/transparency/click-through/resize, lifecycle, error paths); remaining is **real-Argos wiring** when the dev Argos venv is available, then the final close-out record per Definition of Done. The Q1 display policy is resolved (verbatim latest partial as the active line; committed finals as history; translated text replaces source when completed).
...
Slice 5 close-out: **real-Argos wiring** verification when the dev Argos venv is available (record evidence in `docs/reports/TEST_REPORT.md`); then Slice 6 — end-to-end latency/accuracy on real audio. See `docs/implementation/BUILD_PLAN.md`.
