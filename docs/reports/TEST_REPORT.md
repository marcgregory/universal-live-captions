...
| Scope | Slice 1 — Audio Capture Spike, Slice 2 — STT Spike, Slice 3 — Translation Spike, Slice 4 — Caption Service (complete), and Slice 5 — Overlay + Control Window (in progress: automated tests green; manual overlay/device verification complete; real-Argos wiring pending) (automated unit tests + manual real-model verification) |
...
- [ ] **Real-Argos wiring: blocked/pending** — this machine has no `argostranslate` on system Python and the dev Argos venv is not currently available, so translation stays Off by default. When the dev venv is available, verify translated text appears on committed lines and source is preserved on failure.
...
**Result: Passed** for overlay/control-window behavior, device enumeration, real capture → Whisper → overlay captions, interaction (move/resize/click-through), lifecycle (stop/restart/clean close), and the model-not-found + source-equals-target error paths. Real-Argos wiring remains pending (dev venv blocked).
...
- Slice 5 manual overlay/device verification **completed 2026-08-01** (recorded in the Slice 5 section above); **real-Argos wiring pending** (dev Argos venv unavailable on this machine); Slice 5 not yet closed out.
