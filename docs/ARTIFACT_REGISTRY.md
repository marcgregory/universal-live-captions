...
| Local Dev Artifacts | `artifacts/` (governed top-level; see [PROJECT_CONSTITUTION.md](PROJECT_CONSTITUTION.md) §1) | Gitignored developer-only caches — sample audio, spike outputs, benchmark report outputs. The directory is canonical; its contents are not release-tracked. (Former Whisper models and Argos venv/packages removed by ADR-0011.) | Bootstrap (de facto) + formalized 2026-08-07 |
