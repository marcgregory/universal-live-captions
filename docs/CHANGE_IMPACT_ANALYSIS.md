...
- **Status:** In progress (2026-08-01) — implementation + unit tests complete: `UniversalCaptions.App` (DI composition root, `IOverlayService`, `CaptionOverlayWindow`, `CaptionPipeline`, `ControlWindow`, `App.xaml.cs`) + `UniversalCaptions.App.Tests` (22 tests); solution builds 0 warnings/0 errors, 190/190 tests pass, `dotnet format --verify-no-changes` clean, no vulnerable packages. Q1 display policy resolved and implemented. **Remaining before close-out:** manual overlay/device verification (real system audio, always-on-top/transparency/click-through/resize), real-Argos wiring when the dev Argos venv is available, fresh-context review of the App code, and the final close-out record. Slice 5 will be marked Completed only when its Definition of Done (incl. recorded manual verification) is satisfied.
