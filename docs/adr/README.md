...
| ADR-0003 | Superseded (ADR-0011) | Streaming `ISpeechToTextEngine` abstraction; local Whisper as first engine | 2026-07-31 |
...
| ADR-0006 | Superseded (ADR-0011) | `ITranslationEngine` abstraction; Argos Translate as first engine (local process) | 2026-07-31 |
...
| ADR-0008 | Superseded (ADR-0011) | Production STT default = faster-whisper native streaming + live partials (supersedes ADR-0003 default-model clause; ggml-base kept as explicit fallback) | 2026-08-05 |
...
| ADR-0010 | Approved | Canonical audio boundary: every Gemini/STT consumer receives mono float32/16 kHz via `UniversalCaptions.Audio` | 2026-08-09 |
| ADR-0011 | **Approved (governing)** | **Gemini-only pipeline: local Whisper + Argos removed; one Gemini Live session produces both transcription and translation (`ILiveAudioTranslationEngine`)** | 2026-08-21 |
